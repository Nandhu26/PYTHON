...
def validateemployeeID():
    while True:
        ID=input('Enter your Employee ID:"Eg:0001" :')
        EmpID= 'ACE'+ID
        if (not(ID.isdigit())):
...
def validateemployeename():
    while True:
        employeenames=input("Enter your Name without spaces:")
...
def validatesalary():
    while True:
        ...
        elif int(salary)<1000 or int(salary) >1000000:
            print("salary range should be between Rs 1000 and Rs.1000000. Please try again")
            continue
        else:
            return salary
